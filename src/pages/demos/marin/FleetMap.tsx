...
export default function FleetMap({
  ...
  const items = useMemo(() => {
    ...
    const score = (c) =>
      c.status === "Iniciado" || c.status === "Alerta"
        ? 3
        : c.aisLat != null && c.aisLon != null && c.aisPosAt
          ? 2
          : 1;
    const byKey = new Map();
    ...
    berth.forEach((c, i) => {
      ...
      const lon =
        MARIN.lon + (BERTH_RADIUS * Math.sin(ang)) / Math.cos((MARIN.lat * Math.PI) / 180);
  ...
  useEffect(() => {
    ...
    const selInCluster =
      clusterBerth &&
      selectedKey != null &&
      berthItems.some((b) => shipKey(b.call) === selectedKey);

    // Boya del puerto (con insignia si se agrupan los atracados).
    const port = L.marker([MARIN.lat, MARIN.lon], {
      icon: marinIcon(clusterBerth ? berthItems.length : 0, selInCluster),
      zIndexOffset: selInCluster ? 2000 : 1000,
    }).addTo(layer);
    port.on("mouseover", () => {
      const p = pt([MARIN.lat, MARIN.lon]);
      setHover({ x: p.x, y: p.y, marin: true });
    });
    port.on("mouseout", () => setHover(null));
    if (clusterBerth)
      port.on("click", () => {
        setHover(null);
        map.flyTo([MARIN.lat, MARIN.lon], CLUSTER_MIN_ZOOM, { duration: 0.8 });
      });
  ...
  useEffect(() => {
    ...
    const target = ais
      ? L.latLng(ais.lat, ais.lon)
      : berth
        ? clusterBerth
          ? L.latLng(MARIN.lat, MARIN.lon)
          : L.latLng(berth.lat, berth.lon)
        : null;
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ position: "relative", isolation: "isolate" }}>
        ...
        {hover && (
          <div
            style={{
              ...
              borderRadius: 8,
              padding: "6px 10px",
              boxShadow: "0 2px 12px rgba(1,11,36,0.2)",
              whiteSpace: "nowrap",
            }}
          >
            {hover.marin ? (
                ...
                {clusterBerth && (
                  <>
                    <div style={{ fontSize: 11, color: C.success, fontWeight: 700, marginTop: 2 }}>
                      {berthItems.length} buque{berthItems.length === 1 ? "" : "s"} atracado
                      {berthItems.length === 1 ? "" : "s"}
                    </div>
                    {berthItems.slice(0, 6).map((b) => (
                      <div key={b.call.id} style={{ fontSize: 10, color: C.gray }}>
                        • {b.call.name}
                      </div>
                    ))}
                    {berthItems.length > 6 && (
                      <div style={{ fontSize: 10, color: C.gray }}>
                        … y {berthItems.length - 6} más
                      </div>
                    )}
              ...
              <>
                ...
                {it.kind === "berth" ? (
                  <>
                    ...
                    {c.berth && c.berth !== "—" && (
                      <div style={{ fontSize: 10, color: C.gray }}>Muelle {c.berth}</div>
                    )}
                    <div style={{ fontSize: 10, color: C.gray, marginTop: 2 }}>
                      ubicación aproximada · en puerto por dato de la AP
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    {c.aisDestination && (
                      <div style={{ fontSize: 11, color: C.gray }}>→ {c.aisDestination}</div>
                    )}
                    {c.aisPosAt && (
                      <div style={{ fontSize: 10, color: C.gray, marginTop: 2 }}>
        ...
        {!items.length && (
          ...
          >
            Sin buques que mostrar en el mapa con el filtro actual (posiciones AIS obsoletas se
            descartan; los atracados aparecen en el puerto).
          </div>
        )}
      </div>
    </div>
