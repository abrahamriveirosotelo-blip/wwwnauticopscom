...
const MARIN = { lat: 42.4053, lon: -8.702, label: "Puerto de Marín" };
const APPROACH = { lat: 42.4, lon: -8.752 }; // aproximación por la ría (O): por aquí entran/salen
...
export default function SchedulePlayback({
  calls,
  onSelect,
  selectedId = null,
  isMobile = false,
  ...
  const { tStart, tEnd, ships, dayTicks } = useMemo(() => {
    ...
    for (const s of withT) {
      if (s.arr != null) times.push(s.arr);
      if (s.dep != null) times.push(s.dep);
    }
  ...
  const shipStateAt = (s, T) => {
    ...
    const bearing = (from, to) =>
      (Math.atan2((to.lon - from.lon) * Math.cos((MARIN.lat * Math.PI) / 180), to.lat - from.lat) *
        180) /
      Math.PI;
    const easeOut = (f) => 1 - Math.pow(1 - f, 3); // desacelera al final (atracar)
    const easeIn = (f) => f * f * f; // acelera despacio al principio (zarpar)
    if (arr != null && T < arr) {
      // entrando (bocana → atraque): desacelera al llegar
      const e = easeOut(Math.min(1, Math.max(0, (T - (arr - GLIDE_MS)) / GLIDE_MS)));
      return {
        visible: true,
        lat: lerp(APPROACH.lat, berth.lat, e),
        lon: lerp(APPROACH.lon, berth.lon, e),
        phase: "in",
        deg: bearing(APPROACH, berth),
        color: C.cyan,
      };
    }
    if (dep != null && T > dep) {
      // saliendo (atraque → bocana): arranca lento y acelera
      const e = easeIn(Math.min(1, Math.max(0, (T - dep) / GLIDE_MS)));
      return {
        visible: true,
        lat: lerp(berth.lat, APPROACH.lat, e),
        lon: lerp(berth.lon, APPROACH.lon, e),
        phase: "out",
        deg: bearing(berth, APPROACH),
        color: C.warning,
      };
    }
    return {
      ...
      lon: berth.lon,
      phase: "dock",
      deg: bearing(berth, MARIN),
      color: C.success,
    }; // atracado (proa hacia el puerto)
  };
  ...
  useEffect(() => {
    ...
    darkLayerRef.current = L.tileLayer(
      ...
      { attribution: cartoAttr, subdomains: "abcd", opacity: darknessRef.current },
    );
  ...
  useEffect(() => {
    ...
    for (const s of ships) {
      const m = L.marker([s.berth.lat, s.berth.lon], {
        icon: playIcon(C.success, null, false, s.call.name),
        opacity: 0,
      }).addTo(layer);
  ...
  const avisoBands = useMemo(
    () =>
      avisos.map((a) => ({
        ...a,
        d0: new Date(a.desde).getTime(),
        d1: new Date(a.hasta).getTime(),
      })),
    [avisos],
  );
  ...
  const darkness = useMemo(
    () => (ships.length ? Math.round(nightFactor(sunT) * 100) / 100 : 0),
    [sunT, ships.length],
  );
  ...
  useEffect(() => {
    const map = mapRef.current,
      layer = darkLayerRef.current;
    if (!map || !layer) return;
    if (darkness > 0) {
      if (!map.hasLayer(layer)) layer.addTo(map);
      layer.setOpacity(darkness);
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: C.gray }}>
          entrada / salida según ETA · ETD · movimiento esquemático
        </div>
      </div>
      ...
      {ships.length > 0 && (
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10 }}>
            ...
            <div
              ref={trackRef}
              className="marin-slider"
              role="slider"
              tabIndex={0}
              aria-label="Línea de tiempo de la simulación"
              aria-valuemin={Math.round(tStart)}
              aria-valuemax={Math.round(tEnd)}
            ...
            >
              ...
              {avisoBands.map((a) => {
                ...
                return (
                  <div
                    key={`${a.desde}-${a.hasta}-${a.nivel}-${a.fenomeno}`}
                    aria-hidden="true"
                    style={{
                      position: "absolute",
                      top: 0,
                  ...
                  >
                    <div
                      style={{ position: "absolute", inset: 0, background: col, opacity: 0.22 }}
                    />
                    <div
                      style={{
                        position: "absolute",
              ...
              {dayTicks.map((tk) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 9, color: C.gray, marginTop: 1 }}>
                      {new Date(tk).getDate()}
                    </span>
                  </div>
                );
              ...
              {activeAviso &&
                (() => {
                  const base = {
                    ...
                    borderRadius: 6,
                    whiteSpace: "nowrap",
                    background: nivelColor(activeAviso.nivel),
                    color: activeAviso.nivel === "amarillo" ? "#3a2e00" : "#fff",
                  };
                  ...
                  return onAvisoClick ? (
                    <button
                      type="button"
                      onClick={() => onAvisoClick(activeAviso)}
                    ...
                    <span title={`Aviso AEMET · nivel ${activeAviso.nivel}${desc}`} style={base}>
                      {label}
                    </span>
                  );
                })()}
              <span style={{ fontSize: 11, fontWeight: 700, color: docked ? C.success : C.gray }}>
                ⚓ {docked} en puerto
              </span>
            </span>
          </div>
