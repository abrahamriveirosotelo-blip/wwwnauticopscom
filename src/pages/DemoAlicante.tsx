...
const TUG = { tugboat:"HÉRCULES APA", patron:"J. Martínez", mecanico:"R. Soler", marinero:"P. Ruiz",
  ir_at:"07:02", cos_at:"07:18", rc_at:"07:21", sc_at:"07:52", fr_at:"08:05", power_pct:75 };
          ...
          {call.status !== "Prevista" && (
            <div style={{marginBottom:20}}>
              ...
              <div style={{background:B.offWhite,borderRadius:12,border:`1px solid ${B.grayLight}`,overflow:"hidden"}}>
                {[
                  { label:"Atracado",             done:true,  inProgress:false, time:"11/05 · 08:42", by:"Práctico J. Morales" },
                  { label:"Inicio de operaciones", done:!isAlert, inProgress:isAlert, time:isAlert?"En curso":"11/05 · 09:05", by:isAlert?"Agente: A. Pérez":"Agente: A. Pérez" },
                  { label:"Fin de operaciones",    done:false, inProgress:false, time:null, by:null },
                  { label:"Desatracado",           done:false, inProgress:false, time:null, by:null },
                ].map((m, i, arr) => {
                  const icon = m.done ? "✅" : m.inProgress ? "🔄" : "⌛";
                  ...
                  return (
                    <div key={m.label} style={{
                      ...
                      borderLeft: m.inProgress ? `3px solid ${B.warning}` : "3px solid transparent",
                    }}>
                      <span style={{fontSize:16, flexShrink:0}}>{icon}</span>
                      <div style={{flex:1, minWidth:0}}>
                        <div style={{fontSize:12, fontWeight:700,
                          color: m.done ? B.navy : m.inProgress ? "#92400E" : B.gray}}>
                          {m.label}
                        </div>
                        {m.by && (
                      ...
                      <div style={{textAlign:"right", flexShrink:0}}>
                        {m.time ? (
                          <div style={{fontSize:11, fontWeight:700,
                            color: m.done ? B.success : m.inProgress ? B.warning : B.gray,
              ...
              <div style={{background:B.offWhite,borderRadius:12,padding:18,border:`1px solid ${B.grayLight}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                  ...
                  <span style={{padding:"3px 10px",borderRadius:99,fontSize:9,fontWeight:800,
                    ...
                    {isAlert?"⚠ EN CURSO":"COMPLETADO"}
                  </span>
                </div>
                <div style={{marginBottom:16}}>
                  {[["IR","Salida base",TUG.ir_at,true],["COS","Al costado",TUG.cos_at,true],
                    ["RC","Recoge cabo",isAlert?"—":TUG.rc_at,!isAlert],
                    ["SC","Sale costado",isAlert?"—":TUG.sc_at,!isAlert],
                    ["FR","Llega a base",isAlert?"—":TUG.fr_at,!isAlert]
                  ].map(([c,l,t,d],i,a)=>(
                    <TugStep key={c} code={c} label={l} time={t} done={d} last={i===a.length-1}/>
                  ))}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                  {[["Patrón",TUG.patron],["Mecánico",TUG.mecanico],["Marinero",TUG.marinero]].map(([r,n])=>(
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {filtered.map((c,i)=>{
                ...
                const isAffected = ["NIEVES B","SPIRIT"].includes(c.name) && c.berth==="23";
                const affectRisk = c.name==="NIEVES B" ? { label:"Impacto ALTO", color:"#DC2626", bg:"#FEE2E2" }
                                 : c.name==="SPIRIT"   ? { label:"Impacto MEDIO", color:"#D97706", bg:"#FEF3C7" }
                                 : null;
                return (
                  <tr key={c.id}
