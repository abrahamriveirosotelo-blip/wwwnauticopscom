...
function mapStatus(navStatus) {
  ...
  if (!s) return "";
  if (s.includes("under way") || s.includes("underway") || s.includes("sailing"))
    return "Navegando";
...
function applyLive(call, live, scrapedAt) {
  ...
  call.aisArrivedMarin =
    call.aisAtMarin &&
    (call.aisStatus === "Atracado" || call.aisStatus === "Fondeado") &&
    !call.aisEta;
}
...
async function main() {
  ...
  if (vIdx !== -1) {
    ...
    console.log(
      "→ estado:",
      mapStatus(live.navStatus),
      "| ETA España:",
      aisEtaToSpainIso(live.aisEta) || "—",
      "| atMarin:",
      destIsMarin(live.destination),
    );
    return;
  }
  ...
  for (const [imo, detailId] of idByImo) {
    try {
      ...
      if (!liveData.navStatus && !liveData.destination) {
        console.warn(
          `⚠️  ${imo}: ficha sin datos AIS legibles (¿cambio de layout?) — no se aplica`,
        );
        continue;
      }
      ...
      console.log(
        `✓ ${imo} → ${st || "—"}${liveData.speed ? ` · ${liveData.speed} kn` : ""}${liveData.draught ? ` · calado ${liveData.draught} m` : ""}${liveData.aisEta ? ` · ETA AIS ${liveData.aisEta}` : ""}`,
      );
    } catch (err) {
